...
class CalendarSettingsTab extends obsidian.PluginSettingTab {
    ...
    addHeatmapMetricSetting() {
        new obsidian.Setting(this.containerEl)
            .setName("热力图显示依据")
            .setDesc("选择颜色深浅按当天新增笔记数量，还是按当天新增字数。")
            .addDropdown((dropdown) => {
            dropdown.addOption("words", "新增字数");
            dropdown.addOption("count", "新增笔记数量");
    ...
    addIncludeDailyNotesInHeatmapSetting() {
        new obsidian.Setting(this.containerEl)
            .setName("包含 Daily Note")
            .setDesc("开启后，新增笔记数量和新增字数都会把 Daily Note 计入热力图。")
            .addToggle((toggle) => {
            toggle.setValue(this.plugin.options.includeDailyNotesInHeatmap !== false);
            toggle.onChange(async (value) => {
                await this.plugin.writeOptions(() => ({
                    includeDailyNotesInHeatmap: value
                }));
            });
        });
    }
...
function getHeatmapDateKeyFromDailyNote(file) {
    const dateMatch = file.basename.match(/^(\d{4}-\d{2}-\d{2})/);
    if (!dateMatch) {
        return null;
    }
    const date = window.moment(dateMatch[1], "YYYY-MM-DD", true);
    return date.isValid() ? date.format("YYYY-MM-DD") : null;
}
function getHeatmapDateKeyFromFrontmatter(fileContents) {
    const frontmatterMatch = (fileContents || "").match(/^---\s*\n([\s\S]*?)\n---/);
    if (!frontmatterMatch) {
        return null;
    }
    const createdMatch = frontmatterMatch[1].match(/^created:\s*["']?(\d{4}-\d{2}-\d{2})/m);
    if (!createdMatch) {
        return null;
    }
    const date = window.moment(createdMatch[1], "YYYY-MM-DD", true);
    return date.isValid() ? date.format("YYYY-MM-DD") : null;
}
function getHeatmapDateKey(file, fileContents, isDailyNote) {
    if (isDailyNote) {
        return getHeatmapDateKeyFromDailyNote(file);
    }
    return getHeatmapDateKeyFromFrontmatter(fileContents) || window.moment(file.stat.ctime).format("YYYY-MM-DD");
}
...
const createdNotesHeatmapSource = {
    getDailyMetadata: async (date) => {
        ...
        const tooltip = `新增 ${stat.count} 篇笔记，新增 ${stat.words} 字`;
        return {
